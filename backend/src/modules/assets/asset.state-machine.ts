...
export const ASSET_TRANSITIONS: Transition[] = [
  ...
  { from: 'ALLOCATED',          to: 'AVAILABLE',          triggeredBy: 'allocation.return',          allowedRoles: ['ADMIN','ASSET_MANAGER','HOLDER_OR_OWNER_RESOLVED_AT_RUNTIME'] },
  ...
  { from: 'RESERVED',           to: 'AVAILABLE',          triggeredBy: 'booking.cancel',             allowedRoles: ['ADMIN','ASSET_MANAGER','HOLDER_OR_OWNER_RESOLVED_AT_RUNTIME'] },
...
export function assertValidTransition(currentStatus: AssetStatus, targetStatus: AssetStatus, event: string, actorRole: string) {
  const match = ASSET_TRANSITIONS.find(t =>
    t.from === currentStatus && t.to === targetStatus && t.triggeredBy === event
  );
  if (!match) {
    throw new ApiError('INVALID_STATE_TRANSITION', 409, `Cannot move asset from ${currentStatus} to ${targetStatus} via ${event}`);
  }
